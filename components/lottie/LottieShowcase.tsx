...
export default function LottieShowcase() {
  ...
  return (
    <section aria-labelledby="lottie-heading" className="bg-zinc-950 px-6 py-24">
      <h2
        id="lottie-heading"
...
function AnimationCard({
  ...
  const setDotLottieRef = useCallback(
    (instance: DotLottie | null) => {
      ...
      // If reduced motion is requested, pause on the first frame.
      if (reducedMotion) {
        instance.pause();
        return;
      }

      // For non-loop triggers, start paused so the user can interact.
  ...
  const isInteractive = config.trigger === "click" || config.trigger === "hover";
